...
# Add the SPINO directory to the path
spino_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../..'))
sys.path.insert(0, spino_path)

# Add the panoptic_segmentation_model directory to the path for its internal imports
panoptic_seg_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', 'panoptic_segmentation_model'))
sys.path.insert(0, panoptic_seg_path)

# Import from panoptic_segmentation_model with adjusted path
from external.ms_deformable_attention.modules import MSDeformAttn
from panoptic_label_generator.models.vit.vision_transformer import DinoVisionTransformer
...
class ViTAdapter(DinoVisionTransformer):
    def __init__(self, pretrain_size=224, conv_inplane=64, n_points=4,
                 ...
                 use_extra_extractor=True, with_cp=False, use_checkpoint=False,
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.interactions):
            ...
            if self.use_checkpoint:
                x, c, cls = cp.checkpoint(
                    layer, x, c, cls, self.blocks[indexes[0]:indexes[-1] + 1],
