...
# Resize position embedding from 1370 to 197 positions (14x14+1 -> 16x16+1)
if 'pos_embed' in model:
    pos_embed = model['pos_embed']  # [1, 1370, 768]
    if args.debug:
    ...
    # 1369 = 37*37 (DinoV2 uses 37x37 grid for 14x14 patches)
    spatial_pos = spatial_pos.reshape(1, 37, 37, 768).permute(0, 3, 1, 2)  # [1, 768, 37, 37]
    
    # Interpolate to 14x14 grid (for 16x16 patches, we need 14x14 spatial positions)
    spatial_pos = F.interpolate(spatial_pos, size=(14, 14), mode='bilinear', align_corners=False)
    spatial_pos = spatial_pos.permute(0, 2, 3, 1).reshape(1, 196, 768)  # [1, 196, 768]
    
    # Concatenate class token and spatial positions
    new_pos_embed = torch.cat([cls_pos, spatial_pos], dim=1)  # [1, 197, 768]
